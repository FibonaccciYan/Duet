...
if [[ "${model_type}" == "sdar" ]]; then
  model_args="pretrained=${model},trust_remote_code=true,dtype=${DTYPE:-float16},attn_implementation=${ATTN_IMPLEMENTATION:-sdpa},sparse_dlm=${sparse_dlm},query_sparse=${query_sparse},prefix_sparse=${prefix_sparse},prefix_token_budget=${prefix_budget},prefix_chunk_size=${PREFIX_CHUNK_SIZE:-256},losa=${losa},losa_active_topk=${losa_active_topk},sparse_dlm_ratio=${SPARSE_DLM_RATIO:-0.5},sparse_dlm_top_k=${SPARSE_DLM_TOP_K:-64},sparse_dlm_selection_interval=${SPARSE_DLM_SELECTION_INTERVAL:-1},sparse_dlm_dense_fallback_mask_count=${SPARSE_DLM_DENSE_FALLBACK_MASK_COUNT:-0},sparse_dlm_refresh_step=${SPARSE_DLM_REFRESH_STEP:--1},sparse_dlm_selection_layer=${SPARSE_DLM_SELECTION_LAYER:-5},sparse_dlm_deep_only_transfer=${SPARSE_DLM_DEEP_ONLY_TRANSFER:-false},moe_expert_patch=${MOE_EXPERT_PATCH:-false},block_length=${block_length},steps=${steps},temperature=${TEMPERATURE:-0.0},threshold=${threshold},remasking_strategy=${REMASKING_STRATEGY:-sequential},eb_threshold=${EB_THRESHOLD:-0.35},mask_id=${mask_id}"
  if [[ -n "${EOS_ID:-}" ]]; then
