...
if [[ "${model_type}" == "sdar" ]]; then
  model_args="pretrained=${model},trust_remote_code=true,dtype=${DTYPE:-float16},attn_implementation=${ATTN_IMPLEMENTATION:-sdpa},sparse_dlm=${sparse_dlm},query_sparse=${query_sparse},prefix_sparse=${prefix_sparse},prefix_token_budget=${prefix_budget},prefix_chunk_size=${PREFIX_CHUNK_SIZE:-256},losa=${losa},losa_active_topk=${losa_active_topk},losa_score_mode=${losa_score_mode},losa_key_samples=${losa_key_samples},sparse_dlm_ratio=${SPARSE_DLM_RATIO:-0.5},sparse_dlm_top_k=${SPARSE_DLM_TOP_K:-64},sparse_dlm_selection_interval=${SPARSE_DLM_SELECTION_INTERVAL:-1},query_dense_threshold=${QUERY_DENSE_THRESHOLD:-0},sparse_dlm_refresh_step=${SPARSE_DLM_REFRESH_STEP:--1},sparse_dlm_selection_layer=${SPARSE_DLM_SELECTION_LAYER:-5},sparse_dlm_deep_only_transfer=${SPARSE_DLM_DEEP_ONLY_TRANSFER:-false},moe_expert_patch=${MOE_EXPERT_PATCH:-false},block_length=${block_length},steps=${steps},temperature=${TEMPERATURE:-0.0},threshold=${threshold},remasking_strategy=${REMASKING_STRATEGY:-sequential},eb_threshold=${EB_THRESHOLD:-0.35},mask_id=${mask_id}"
  if [[ -n "${EOS_ID:-}" ]]; then
    model_args+=",eos_id=${EOS_ID}"
  fi
else
  model_args="pretrained=${model},trust_remote_code=true,dtype=${DTYPE:-bfloat16},attn_implementation=${ATTN_IMPLEMENTATION:-sdpa},sparse_dlm=${sparse_dlm},sparse_dlm_ratio=${SPARSE_DLM_RATIO:-0.7},sparse_dlm_top_k=${SPARSE_DLM_TOP_K:-64},sparse_dlm_selection_interval=${SPARSE_DLM_SELECTION_INTERVAL:-4},query_dense_threshold=${QUERY_DENSE_THRESHOLD:-4},sparse_dlm_selection_layer=${SPARSE_DLM_SELECTION_LAYER:-1},query_sparse=${query_sparse},prefix_sparse=${prefix_sparse},prefix_token_budget=${prefix_budget},prefix_chunk_size=${PREFIX_CHUNK_SIZE:-256},losa=${losa},losa_active_topk=${losa_active_topk},losa_score_mode=${losa_score_mode},losa_key_samples=${losa_key_samples},query_losa_union=${QUERY_LOSA_UNION:-false},moe_expert_patch=${MOE_EXPERT_PATCH:-true},block_length=${block_length},steps=${steps},temperature=${TEMPERATURE:-0.0},threshold=${threshold},editing_threshold=${EDITING_THRESHOLD:-0.0},num_to_transfer=${NUM_TO_TRANSFER:-1},mask_id=${mask_id},eos_id=${eos_id}"
fi
