"""Experimental exact-shape SDAR decoder replay with explicit KV state I/O."""
import time
import types
import torch
...
class SparseLayerGraph:
    ...
    def __call__(self,hidden,**kwargs):
        ...
        inputs=[hidden,source_key,source_value,kwargs["position_ids"],
                *kwargs["position_embeddings"]]
        if self.sparse:
            inputs.append(source.positions)
        for name in ("attention_mask","cache_position"):
            if isinstance(kwargs.get(name),torch.Tensor):
        ...
        with torch.cuda.stream(self.stream):
            self.load(hidden,kwargs)
            sync_mode=getattr(self.layer,"_sparse_graph_sync_mode","both")
            if getattr(self.layer,"_sparse_graph_sync_probe",False) and sync_mode in ("both","before"):
                torch.cuda.synchronize()
            self.graph.replay()
            if getattr(self.layer,"_sparse_graph_sync_probe",False) and sync_mode in ("both","after"):
                torch.cuda.synchronize()
            if self.sparse:
                source_key[...,self.prefix:,:].copy_(self.key[...,self.prefix:,:])
                source_value[...,self.prefix:,:].copy_(self.value[...,self.prefix:,:])
            result=copy_tree(self.output)
