"""Paper-faithful LoSA attention patch.

This module patches only the loaded model object in memory and delegates the
LoSA semantics to the operators vendored in :mod:`src.losa.operators`.
"""
...
from .operators import losa_attention_step
...
def _run_losa_attention(
    *,
    ...
    trace_item = {
        ...
        "union_pages": [
            {
                "kv_head": int(union.kv_head),
                "num_pages": int(union.pages.numel()),
            }
...
def install_paper_losa_attention(model, family: str) -> None:
