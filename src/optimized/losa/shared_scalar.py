...
def _shared_scalar(Q,K,V,P,O,L, QR:tl.constexpr,QH:tl.constexpr,QD:tl.constexpr,
    ...
    for start in tl.range(0,tokens,16,num_stages=1):
        ...
        if SHARE > 0:
            acc0=acc0*alpha0+tl.sum(p0[:,None]*value,axis=0)
            z0=z0*alpha0+tl.sum(p0,axis=0)
            m0=nm0
        if SHARE > 1:
            acc1=acc1*alpha1+tl.sum(p1[:,None]*value,axis=0)
            z1=z1*alpha1+tl.sum(p1,axis=0)
            m1=nm1
        if SHARE > 2:
            acc2=acc2*alpha2+tl.sum(p2[:,None]*value,axis=0)
            z2=z2*alpha2+tl.sum(p2,axis=0)
            m2=nm2
        if SHARE > 3:
            acc3=acc3*alpha3+tl.sum(p3[:,None]*value,axis=0)
            z3=z3*alpha3+tl.sum(p3,axis=0)
            m3=nm3
        if SHARE > 4:
            acc4=acc4*alpha4+tl.sum(p4[:,None]*value,axis=0)
            z4=z4*alpha4+tl.sum(p4,axis=0)
            m4=nm4
        if SHARE > 5:
            acc5=acc5*alpha5+tl.sum(p5[:,None]*value,axis=0)
            z5=z5*alpha5+tl.sum(p5,axis=0)
            m5=nm5
        if SHARE > 6:
            acc6=acc6*alpha6+tl.sum(p6[:,None]*value,axis=0)
            z6=z6*alpha6+tl.sum(p6,axis=0)
            m6=nm6
        if SHARE > 7:
            acc7=acc7*alpha7+tl.sum(p7[:,None]*value,axis=0)
            z7=z7*alpha7+tl.sum(p7,axis=0)
            m7=nm7
...
def attention(q,k,v,pages,*,page_size,prefix_length,softmax_scale=None,workspace=None,share=2,num_warps=4):
    a,h,d=q.shape
    hk=k.shape[1];g=h//hk
