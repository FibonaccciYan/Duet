"""Two-stage shared-KV pipeline preserving v5 D128/page16 FP32 reductions.

Stage 1 shares K loads across score_share queries, writes unscaled scores.
Stage 2 shares the resulting workspace and optionally V loads, preserving
16-token online-softmax order and explicit FP32 FMA reduction ordering.
This changes execution only, never selected pages or their order.
"""
import torch
import triton
...
@triton.jit
def _scores(Q,K,P,S,QR:tl.constexpr,QH:tl.constexpr,QD:tl.constexpr,
            KT:tl.constexpr,KH:tl.constexpr,KD:tl.constexpr,PH:tl.constexpr,PS:tl.constexpr,
            A:tl.constexpr,H:tl.constexpr,G:tl.constexpr,D:tl.constexpr,N,SCALE,
            PAGE:tl.constexpr,CAP:tl.constexpr,BD:tl.constexpr,SHARE:tl.constexpr):
    head,tile,chunk=tl.program_id(0),tl.program_id(1),tl.program_id(2)
    d=tl.arange(0,BD)
    n=chunk*16+tl.arange(0,16)
    page=tl.load(P+head*PH+(n//PAGE)*PS,mask=n//PAGE<CAP,other=-1)
    token=page*PAGE+n%PAGE
    valid=(page>=0)&(token<N)&(n<CAP*PAGE)
    key=tl.load(K+token[:,None]*KT+head*KH+d[None,:]*KD,mask=valid[:,None]&(d[None,:]<D),other=0.).to(tl.float32)
    if SHARE > 0:
        row=(tile*SHARE+0)//G
        qh=head*G+(tile*SHARE+0)%G
        q=tl.load(Q+row*QR+qh*QH+d*QD,mask=(row<A)&(d<D),other=0.).to(tl.float32)
        score=tl.sum(key*q[None,:],axis=1)
        score=tl.where(valid,score,-float("inf"))
        tl.store(S+(row*H+qh)*(CAP*PAGE)+n,score,mask=(row<A)&(n<CAP*PAGE))
    if SHARE > 1:
        row=(tile*SHARE+1)//G
        qh=head*G+(tile*SHARE+1)%G
        q=tl.load(Q+row*QR+qh*QH+d*QD,mask=(row<A)&(d<D),other=0.).to(tl.float32)
        score=tl.sum(key*q[None,:],axis=1)
        score=tl.where(valid,score,-float("inf"))
        tl.store(S+(row*H+qh)*(CAP*PAGE)+n,score,mask=(row<A)&(n<CAP*PAGE))
...
@triton.jit
def _values(Q,K,V,P,O,L,S, QR:tl.constexpr,QH:tl.constexpr,QD:tl.constexpr,
    ...
    for start in tl.range(0,tokens,16,num_stages=1):
        ...
        if SHARE > 0:
            score0=tl.load(S+(row0*H+qh0)*(CAP*PAGE)+packed,mask=(row0<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m0=nm0
        if SHARE > 1:
            score1=tl.load(S+(row1*H+qh1)*(CAP*PAGE)+packed,mask=(row1<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m1=nm1
        if SHARE > 2:
            score2=tl.load(S+(row2*H+qh2)*(CAP*PAGE)+packed,mask=(row2<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m2=nm2
        if SHARE > 3:
            score3=tl.load(S+(row3*H+qh3)*(CAP*PAGE)+packed,mask=(row3<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m3=nm3
        if SHARE > 4:
            score4=tl.load(S+(row4*H+qh4)*(CAP*PAGE)+packed,mask=(row4<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m4=nm4
        if SHARE > 5:
            score5=tl.load(S+(row5*H+qh5)*(CAP*PAGE)+packed,mask=(row5<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m5=nm5
        if SHARE > 6:
            score6=tl.load(S+(row6*H+qh6)*(CAP*PAGE)+packed,mask=(row6<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
            ...
            m6=nm6
        if SHARE > 7:
            score7=tl.load(S+(row7*H+qh7)*(CAP*PAGE)+packed,mask=(row7<A)&(packed<CAP*PAGE),other=-float("inf"))*SCALE
...
def attention(q,k,v,pages,*,page_size,prefix_length,softmax_scale=None,workspace=None,score_share=4,value_share=2):
    a,h,d=q.shape;hk=k.shape[1];g=h//hk;cap=pages.shape[1]
    if d != 128 or page_size != 16 or k.shape != v.shape or pages.ndim != 2 or pages.shape[0] != hk:
        raise ValueError("shared pipeline requires D128/page16 and matching GQA KV")
    if prefix_length <= 0 or prefix_length > k.shape[0] or cap <= 0:
        raise ValueError("invalid prefix/page capacity")
    if h%hk or score_share not in (1,2,4,8) or value_share not in (1,2,4,8):raise ValueError("bad tiling")
    scores=(torch.empty((a,h,cap*page_size),device=q.device,dtype=torch.float32) if workspace is None else workspace.get("qk_scores",(a,h,cap*page_size),q,torch.float32))
    ...
    _scores[(hk,triton.cdiv(a*g,score_share),triton.cdiv(cap*page_size,16))](q,k,pages,scores,*q.stride(),*k.stride(),*pages.stride(),A=a,H=h,G=g,D=d,N=prefix_length,SCALE=scale,PAGE=page_size,CAP=cap,BD=triton.next_power_of_2(d),SHARE=score_share,num_warps=4)
    _values[(hk,triton.cdiv(a*g,value_share))](q,k,v,pages,out,lse,scores,*q.stride(),*k.stride(),*v.stride(),*pages.stride(),A=a,H=h,G=g,D=d,N=prefix_length,SCALE=scale,PAGE=page_size,CAP=cap,CB=triton.next_power_of_2(cap),BD=triton.next_power_of_2(d),SHARE=value_share,num_warps=4)
    return out,lse
