"""Triton fast paths for optimized LoSA.
...
if triton is not None:
    ...
    def _paged_prefix_attention_kernel(
        ...
        # Compaction stores all selected pages first and pads the tail with -1.
        # Skip only that tail; valid-page order and reductions stay unchanged.
        slots = tl.arange(0, COUNT_BLOCK)
        page_ids = tl.load(pages_ptr + kv_head * stride_pt + slots * stride_ps,
                           mask=slots < MAX_PAGES, other=-1)
        valid_pages = tl.sum((page_ids >= 0).to(tl.int32))
        max_tokens = valid_pages * PAGE_SIZE
        ...
        for token_start in tl.range(0, max_tokens, BLOCK_N, num_stages=1):
            ...
            value_offset = (token[:, None] * stride_vt
                            + kv_head * stride_vh + d[None, :] * stride_vd)
            value = tl.load(v_ptr + value_offset, mask=load_mask, other=0.0).to(
                tl.float32
            )
...
def locality_scores_triton(
    ...
    output = (torch.empty(rows, device=query.device, dtype=torch.float32) if workspace is None
              else workspace.get("locality", (rows,), query, torch.float32))
...
def select_active_rows_triton(
    ...
    locality = locality_scores_triton(query, previous_query, workspace)
    count = min(int(active_count), query.shape[0])
    active = (locality.topk(count, sorted=False).indices.sort().values if workspace is None
              else workspace.active(locality, count))
...
def quest_group_mean_scores_triton(
    ...
    scores = (torch.empty((rows, num_kv_heads, num_pages), device=q.device, dtype=torch.float32)
              if workspace is None else workspace.get("scores", (rows, num_kv_heads, num_pages), q, torch.float32))
...
def compact_prefix_attention_triton(
    ...
    *,
    ...
    # Read the original immutable prefix views; never copy the whole history.
    compact_pages = compact_pages.contiguous()
    output = (torch.empty_like(q_active) if workspace is None else workspace.get("prefix_out", q_active.shape, q_active))
    lse = (torch.empty((rows, num_q_heads), device=q_active.device, dtype=torch.float32)
           if workspace is None else workspace.get("prefix_lse", (rows, num_q_heads), q_active, torch.float32))
    block_d = triton.next_power_of_2(head_dim)
    ...
    _paged_prefix_attention_kernel[grid](
        ...
        PAGE_SIZE=page_size,
        MAX_PAGES=max_pages,
        COUNT_BLOCK=triton.next_power_of_2(max_pages),
        BLOCK_D=block_d,
        BLOCK_N=16,
        num_warps=num_warps,
    )
...
def dense_attention_triton(
    ...
    *,
    ...
    k = k.contiguous()
    v = v.contiguous()
    output = (torch.empty_like(q) if workspace is None else workspace.get("block_out", q.shape, q))
    lse = (torch.empty((rows, num_q_heads), device=q.device, dtype=torch.float32)
           if workspace is None else workspace.get("block_lse", (rows, num_q_heads), q, torch.float32))
    block_d = triton.next_power_of_2(head_dim)
...
def compact_selected_pages_triton(
    ...
    *,
    ...
    if workspace is None:
        page_mask = torch.zeros((num_kv_heads, num_prefix_pages), device=selected_pages.device, dtype=torch.int32)
        output = torch.full((num_kv_heads, max_pages), -1, device=selected_pages.device, dtype=selected_pages.dtype)
    else:
        page_mask = workspace.get("page_mask", (num_kv_heads, num_prefix_pages), selected_pages, torch.int32)
        output = workspace.get("pages", (num_kv_heads, max_pages), selected_pages)
        page_mask.zero_()
        output.fill_(-1)
...
def use_triton_backend(backend: str, *tensors: torch.Tensor) -> bool:
    if backend not in {"auto", "torch", "triton"}:
        raise ValueError(f"unsupported optimized LoSA backend: {backend}")
    if backend == "torch":
        return False
    available = triton_available() and all(tensor.is_cuda for tensor in tensors)
    if backend == "triton" and not available:
        raise RuntimeError("optimized LoSA Triton backend requested but unavailable")
    return available
