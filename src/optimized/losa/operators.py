"""optimized LoSA fixed-shape operators.

The v2 path keeps the paper semantics used by `src.reference.losa`, but removes dynamic
...
@dataclass
class OptimizedLayerState:
    query: torch.Tensor
    prefix_output: torch.Tensor
    prefix_lse: torch.Tensor
    metadata: OptimizedPageMetadata | None


@dataclass
class OptimizedAttentionResult:
    output: torch.Tensor
    lse: torch.Tensor
    prefix_output: torch.Tensor
    prefix_lse: torch.Tensor
    ...
    next_state: OptimizedLayerState
...
def build_page_metadata_reference(k_prefix: torch.Tensor, page_size: int) -> OptimizedPageMetadata:
    if k_prefix.ndim != 3:
    ...
    return OptimizedPageMetadata(k_min, k_max, length, page_size)


def build_page_metadata(k_prefix: torch.Tensor, page_size: int) -> OptimizedPageMetadata:
    if (k_prefix.ndim == 3 and k_prefix.is_cuda and k_prefix.shape[0] > 0
            and 0 < page_size <= 64 and 0 < k_prefix.shape[-1] <= 256
            and k_prefix.dtype in (torch.float16, torch.bfloat16, torch.float32)):
        from src.kernels.optimized.quest_metadata import extrema
        lo, hi = extrema(k_prefix, page_size)
        return OptimizedPageMetadata(lo, hi, k_prefix.shape[0], page_size)
    return build_page_metadata_reference(k_prefix, page_size)
...
def dense_attention(q, k, v, *, softmax_scale=None):
    validate_qkv(q, k, v)
    if (q.is_cuda and q.dtype in (torch.float16, torch.bfloat16)
            and q.shape[0] == 32 and q.shape[-1] == 128
            and (q.shape[1], k.shape[1]) in ((16, 4), (32, 8))):
        from src.kernels.optimized.grouped_fp32 import attention
        return attention(q, k, v, softmax_scale=softmax_scale)
    return dense_attention_reference(q, k, v, softmax_scale=softmax_scale)
...
def quest_page_scores_grouped(
    q: torch.Tensor,
    metadata: OptimizedPageMetadata,
...
def losa_optimized_attention_step(
    query: torch.Tensor,
    k_prefix: torch.Tensor,
    v_prefix: torch.Tensor,
    ...
    *,
    ...
    mode: str = GQAMode.GROUP_MEAN,
    backend: str = "auto",
    metadata: OptimizedPageMetadata | None = None,
    ...
    if previous_state is not None:
        ...
        workspace = getattr(previous_state, "workspace", None)
        if workspace is None:
            workspace = Workspace()
            previous_state.workspace = workspace
    if step_triton_enabled and k_block.shape[0] <= 128:
        block_output, block_lse = dense_attention_triton(
            query, k_block, v_block, workspace=workspace
        )
    else:
    ...
    if previous_state is None:
        ...
        if triton_enabled:
            active_rows, locality = select_active_rows_triton(
                query, previous_state.query, active_count, workspace=workspace
            )
        else:
            active_rows, locality = select_active_rows(
                query, previous_state.query, active_count
            )
        q_active = workspace.get("active_query", (active_rows.numel(), *query.shape[1:]), query)
        torch.index_select(query, 0, active_rows, out=q_active)
        if triton_enabled and mode == GQAMode.GROUP_MEAN:
            scores = quest_group_mean_scores_triton(
                q_active, metadata.k_min, metadata.k_max, workspace=workspace
            )
        else:
            scores = quest_page_scores_grouped(q_active, metadata, mode=mode)
        page_budget = min(metadata.num_pages, math.ceil(token_budget / page_size))
        selected = workspace.topk(scores, page_budget, "selected_pages")
        num_kv_heads = num_kv
        group_size = group
        if triton_enabled:
        ...
        if triton_enabled and shared_kv and dim == 128 and page_size == 16:
            ...
            prefix_output, prefix_lse = attention(
                q_active, k_prefix, v_prefix, compact_pages,
                page_size=page_size, prefix_length=metadata.prefix_length,
                workspace=workspace, score_share=4, value_share=1)
        elif triton_enabled:
        ...
        if fused_state and triton_enabled:
            from .state import finish
            output, lse = finish(query, previous_state, active_rows, prefix_output,
                                 prefix_lse, block_output, block_lse, workspace=workspace)
            previous_state.metadata = metadata
            return OptimizedAttentionResult(
                output=output, lse=lse,
                prefix_output=previous_state.prefix_output,
                prefix_lse=previous_state.prefix_lse,
                block_output=block_output, block_lse=block_lse,
                next_state=previous_state, active_rows=active_rows,
                locality=locality, initialized_dense=False, compact_pages=compact_pages,
            )
