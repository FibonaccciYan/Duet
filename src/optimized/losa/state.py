...
def finish(query,state,active_rows,active_output,active_lse,block_output,block_lse,workspace=None):
    ...
    r,h,d=query.shape
    out=(torch.empty_like(state.prefix_output) if workspace is None else workspace.get("merged_out", state.prefix_output.shape, state.prefix_output))
    lse=(torch.empty_like(state.prefix_lse) if workspace is None else workspace.get("merged_lse", state.prefix_lse.shape, state.prefix_lse))
