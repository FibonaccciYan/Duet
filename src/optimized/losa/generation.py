...
from .attention_patch import install_losa_optimized_attention as install_paper_losa_attention
...
@torch.no_grad()
def build_sdar_prefix_cache(model, x, prefix_length, position_ids, *, block_length, query_chunk_length=4096):
    from src.kernels.optimized.block_causal_prefill import build_prefix
    return build_prefix(model, "sdar", x[:, :prefix_length], position_ids[:, :prefix_length],
                        block_length=block_length, chunk_length=4096)


@torch.no_grad()
def build_llada_prefix_cache(model, x, prefix_length, position_ids, *, block_length, query_chunk_length=4096):
    from src.kernels.optimized.block_causal_prefill import build_prefix
    return build_prefix(model, "llada", x[:, :prefix_length], position_ids[:, :prefix_length],
                        block_length=block_length, chunk_length=4096)
...
def prefix_cache_view(prefix_cache):
    """Disposable cache containers referencing an immutable prefix without copying KV.

    DynamicLayer.update replaces tensors by concatenation; LoSA's fast append
    writes only to its separate combined block buffers, never into this prefix.
    """
    cache = DynamicCache()
...
def _eager_model_forward(model, family, input_ids, attention_mask, position_ids, *, prefix_cache=(), losa_context_kwargs=None, store_kv=True):
    if prefix_cache:
    ...
    cache = prefix_cache_view(prefix_cache) if prefix_cache else None
    kwargs = {
        "input_ids": input_ids,
...
def layerwise_cached_forward(
    ...
    *,
    ...
    base = model.model
    cache = prefix_cache_view(prefix_cache)
    if family == "llada":
...
def block_diffusion_generate(
    ...
    *,
    ...
    if family not in {"llada", "sdar"}:
        raise ValueError("optimized LoSA supports only llada/sdar")
    ...
    for block_idx in range(prefill_blocks, num_blocks):
        ...
        cur_mask = None  # Both supported families construct masks per executed query chunk.
        cur_pos = position_ids[:, :current_window_end]
        if persistent_prefix_cache is not None:
            prefix_cache = persistent_prefix_cache
            # A new block must evaluate its own current input before step zero.
            # Reusing the prefix does not make the previous block's logits valid.
