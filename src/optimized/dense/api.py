...
from src.reference.losa.generation import (
...
def patch_model(
    ...
    **_: Any,
    ...
    from src.reference.sparse import patch_model as patch_sparse_model
    ...
    from .attention_backend import install
    install(model, family)
    from src.kernels.optimized.small_moe import install_small_moe
    model._dense_optimized_small_moe_count = install_small_moe(model)
    if family == "sdar":
        from src.kernels.optimized.sdar_norm import install as install_norm
        model._versioned_direct_norm_count = install_norm(model)
    from src.kernels.optimized.prefix_cache_view import install as install_cache
    model._dense_optimized_generation_globals = install_cache(model)
    if exact_rope:
