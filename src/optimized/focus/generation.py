"""Block-diffusion generation driver for optimized FOCUS."""
...
from src.reference.losa.generation import get_num_transfer_tokens, sample_with_confidence
...
def focus_optimized_generate(
    ...
    *,
    ...
    prefix_cache = build_prefix(
        model, family, x[:, :prefill_blocks * block_length],
        all_positions[:, :prefill_blocks * block_length], block_length=block_length,
    )
    ...
    for block_idx in range(prefill_blocks, num_blocks):
        ...
        block_cache = BlockScratchCache(prefix_cache, block_length)
        post_steps = 0
        step = 0
        while True:
            ...
            if family == "llada" and not bool(active.any().item()):
                ...
                if post_steps > int(max_post_steps):
                    break
            # SDAR has no post-mask editing: stop before a redundant forward.
            # Keep the full-block cache finalization below this loop.
            ...
            result = focus_optimized_forward(
                ...
                mask_id=mask_id,
                alpha=alpha, attention_backend=attention_backend,
            ...
            if family == "llada":
                ...
                if bool(active.any().item()) and resolved <= 0:
                    raise RuntimeError("optimized FOCUS LLaDA mask-to-token step made no progress")
                ...
                resolved = int(active.sum().item()) - int((x[:, start:end] == mask_id).sum().item())
                if bool(active.any().item()) and resolved <= 0:
                    raise RuntimeError("optimized FOCUS SDAR mask-to-token step made no progress")
                state.record_transfer(resolved)
            traces.append(
                {
                    "block": block_idx,
...
__all__ = ["focus_optimized_generate"]
