...
配置来源为 `src/reference/sparse/config.py`，精度评测入口为 `eval_instruct/eval.sh`。
...
```python
from src.reference.sparse import patch_model
...
python scripts/smoke/runtime_mode.py --family llada --mode losa_optimized
python scripts/smoke/runtime_mode.py --family llada --mode focus_optimized
```
...
```text
src/reference/sparse/       SparseDLM 查询/前缀稀疏与共享 CUDA 内核
src/dense/        同生成框架的稠密基线
src/reference/losa/         Paper-LoSA baseline
src/reference/focus/        FOCUS baseline
src/optimized/losa/      最终优化 LoSA v6 runtime（group_mean、workspace、CUDA Graph）
src/optimized/focus/     固定版 FOCUS v8 runtime（LLaDA 默认 FlashAttention）
src/model/        自包含模型适配代码
...
LoSA 当前保留 `src/reference/losa` 原始实现与 `src/optimized/losa` 最终实现；中间 runtime 已移除。历史测速结果仅用于记录当时版本。最终性能测试使用 `scripts/performance/narrative_losa_optimized_dense.py`。
