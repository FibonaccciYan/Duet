...
from src.optimized.losa.shared_ordered import attention
    ...
    for dtype,h,hk in [(torch.float16,32,8),(torch.bfloat16,16,4)]:
        for length in (257,8192,16384,32512):
            for seed in (11,42,20260917):
                ...
                for share,warps in ((1,4),(2,4),(4,4),(2,2)):
                    ws=Workspace()
                    b,bl=attention(q,k,v,pages,share=share,num_warps=warps,workspace=ws,**kw)
                    ...
                    if seed==11:
                        ...
                        record["new_ms"]=bench(lambda:attention(q,k,v,pages,share=share,
                                                               num_warps=warps,workspace=ws,**kw))
                    results.append(record)
                    print(record,flush=True)
    path=Path("results/losa_optimized_20260917/shared_ordered_validation.json")
