"""Real-text two-family optimized Sparse phase matrix; no model algorithm modifications."""
import argparse
import ast
import contextlib
import gc
...
args = p.parse_args()
OUT = Path("results/versioned_optimization_20260916/sparse_profile_matrix")
...
patch_method(model, "sparse_optimized", model_name=args.family, moe_expert_patch=True,
...
for length in args.contexts:
    ...
    result = dict(family=args.family, prompt_tokens=length, generation_budget=32,
                  ...
                  warmup_runs=1, warmup_seconds=warm_seconds, end_to_end_seconds=durations,
    ...
    (OUT / (args.family + ".matrix.json")).write_text(json.dumps(all_results, indent=2))
