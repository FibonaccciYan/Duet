...
case "${model_type}" in
  llada)
    default_model_path="${LLADA_MODEL_PATH:-}"
    default_ratio=0.7
    ...
    ;;
  sdar)
    default_model_path="${SDAR_MODEL_PATH:-}"
...
if [[ -z "${model_path}" ]]; then
  echo "Set MODEL_PATH or the matching LLADA_MODEL_PATH/SDAR_MODEL_PATH" >&2
  exit 2
fi
...
args=(
  scripts/original/smoke/generate.py
  --model "${model_type}"
  --pattern "${pattern}"
