...
FORBIDDEN_RUNTIME_PREFIX = os.environ.get("FORBIDDEN_RUNTIME_PREFIX", "")
DEFAULT_PROMPT = (
...
from src.reference.losa import set_seed


def assert_no_forbidden_sparse_runtime(stage: str) -> None:
    if not FORBIDDEN_RUNTIME_PREFIX:
        return
    bad_paths = [
        item
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=("dense", "losa", "losa_optimized", "focus", "focus_optimized"),
        required=True,
    )
...
def main() -> int:
    ...
    if args.mode in {"dense", "losa", "losa_optimized", "focus_optimized"}:
        runtime_kwargs["moe_expert_patch"] = args.moe_expert_patch
    if args.mode in {"losa", "losa_optimized"}:
    ...
    elif args.mode in {"focus", "focus_optimized"}:
    ...
    if threshold is None:
        threshold = (
            0.95
            if args.family == "sdar" and args.mode in {"focus", "focus_optimized"}
            else 0.85
            if args.family == "sdar"
            else 0.7
        )
    ...
    if args.family == "sdar":
        generation_kwargs.update(
            remasking_strategy=(
                "low_confidence_dynamic" if args.mode in {"focus", "focus_optimized"} else "sequential"
            )
        )
    else:
        generation_kwargs.update(
    ...
    result = {
        ...
        "losa": {
            "enabled": args.mode in {"losa", "losa_optimized"},
            "variant": args.mode,
        ...
        "focus": {
            "enabled": args.mode in {"focus", "focus_optimized"},
            "variant": args.mode,
            "alpha": args.focus_alpha,
            "trace_events": len(generation.trace) if args.mode in {"focus", "focus_optimized"} else 0,
        },
